...
\begin{frame}[fragile]\frametitle{Initialize on First Use}
  In the initialization on first use the variables are static defined locally in a function body:
  \begin{itemize}
  \item They are constructed when the function gets called (they are not constructed when not used);
  \item They are constructed only once, even though the functions are called several times;
  \item They are deleted when the program ends, like other static variables, in reversed order.
  \item Access is thread-safe (since C++11)
  \item<2-> Is this magic?
    \uncover<3->{\alert{NO:} check https://godbolt.org/g/OtyNCe
    }
  \end{itemize}
\end{frame}
