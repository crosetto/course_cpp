...
\begin{frame}[fragile]\frametitle{Example II: Storage}
  ...
  \begin{onlyenv}<1>
    \begin{Cpplisting}[: main\_storage.cpp]{}
int main(){
...
\begin{frame}[fragile]\frametitle{Example II: Storage}
  ...
  constexpr int idx_impl(std::integer_sequence< Indices ... > , Ints ... dims_) const{
    do_something(attach_id(dims_, Indices) ...);
  }
  \end{Cpplisting}
\end{frame}
...
private:
  ...
  \end{Cpplisting}
\uncover<2>{  We need to imlement the \emph{index} method }
\end{frame}
...
constexpr int index(Ints ... coords_) const{
  ...
  \end{Cpplisting}
  \uncover<2>{  We pass on 2 parameter packs: the \emph{coordinates} $n$ and the \emph{indices} $idx$,
    we have to implement the recursive formula
  $$
    \left\{
    \begin{array}{l}
      g_n = g_{n-1} * dims_{n-1} + idx_{n};\\
      g_0 = index_0
    \end{array}
    \right.
  $$
